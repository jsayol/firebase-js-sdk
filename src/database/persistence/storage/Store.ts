...
export class Store {
   ...
   */
  constructor(private database_: string, private store_: string, private storageAdapter_: StorageAdapter) {
  ...
  removePrefixed(prefix: string): Promise<any> {
    return this.storageAdapter_.removePrefixed(this.database_, this.store_, prefix);
  }
