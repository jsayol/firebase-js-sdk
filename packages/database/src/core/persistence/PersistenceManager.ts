...
const boundLog = log.bind(null, 'PersistenceManager:') as (
  ...args: any[]
) => void;
const boundWarn = warn.bind(null, 'PersistenceManager:') as (
  ...args: any[]
) => void;
...
export class PersistenceManager {
  ...
  constructor(
    repo: Repo,
    private cachePolicy_: CachePolicy,
    ...
    this.trackedQueryManager_ = new TrackedQueryManager(
  ...
  applyUserWrite(node: Node, path: Path) {
    if (this.trackedQueryManager_.hasActiveDefault(path)) {
      ...
      this.serverCacheStore_.overwrite(node, path, false).then(() => {
  ...
  */
  ...
  getServerCache(query: Query): Promise<CacheNode> {
    return this.trackedQueryManager_.initialized.then(() => {
      ...
      if (this.trackedQueryManager_.isComplete(query)) {
        ...
        complete = false;
        keysPromise = this.trackedQueryManager_.knownCompleteChildren(
          query.path
        );
      }
      ...
      if (keysPromise) {
        nodePromise = keysPromise.then((keys: string[]) =>
          this.serverCacheStore_.getForKeys(keys, query.path)
        );
      } else {
  ...
  applyServerOverwrite(node: Node, query: Query) {
    ...
    this.serverCacheStore_.overwrite(node, query.path, partial).then(() => {
  ...
  private pruneCheck_() {
    ...
    if (this.cachePolicy_.shouldCheckSize(this.serverUpdatesSincePruneCheck_)) {
      boundLog(
        `reached prune check threshold after ${
          this.serverUpdatesSincePruneCheck_
        } updates. Checking...`
      );
  ...
  private recursivePruneCheck_(isAfterPruning = false): Promise<void> {
    return this.serverCacheStore_.estimatedSize().then((cacheSize: number) => {
      boundLog(
        `cache size${isAfterPruning ? ' after pruning' : ''}: ${cacheSize}`
      );

      if (
        this.cachePolicy_.shouldPrune(
          cacheSize,
          this.trackedQueryManager_.numPrunableQueries()
        )
      ) {
        const pruneForest = this.trackedQueryManager_.pruneOld(
          this.cachePolicy_
        );
        if (pruneForest.prunesAnything()) {
          return this.serverCacheStore_
            .pruneCache(pruneForest, Path.Empty)
