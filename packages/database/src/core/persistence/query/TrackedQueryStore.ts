...
const boundLog = log.bind(null, 'TrackedQueryStore:') as (
  ...args: any[]
) => void;
const boundWarn = warn.bind(null, 'TrackedQueryStore:') as (
  ...args: any[]
) => void;
...
export class TrackedQueryStore {
    ...
    return this.store_
      ...
      .then((trackedQueries: KeyValueItem<TrackedQueryJSON>[]) => {
        ...
        trackedQueries.forEach((item: KeyValueItem<TrackedQueryJSON>) => {
          ...
          if (!trackedQuery) {
            boundWarn(
              `failed to restore tracked query key="${item.key}". Removing it.`
            );
  ...
  save(trackedQuery: TrackedQuery) {
    this.store_
      .set(storeQueryPrefix(trackedQuery.id), trackedQuery.toJSON())
      .then(() => {
  ...
  setKeys(id: number, keys: string[]) {
    ...
    this.store_
      ...
      .then(() => {
        if (added + removed > 0) {
          boundLog(
            `setKeys added=${added} removed=${removed} tracked keys id=${id}`
          );
        }
      })
      .catch((error: Error) => {
  ...
  updateKeys(id: number, addedKeys: string[], removedKeys: string[]) {
    ...
    removedKeys.forEach((key: string) =>
      writeBatch.remove(storeQueryKeysKeyPrefix(id, key))
    );
    addedKeys.forEach((key: string) =>
      writeBatch.set(storeQueryKeysKeyPrefix(id, key), key)
    );
    ...
    writeBatch
      ...
      .then(() => {
        boundLog(
          `updateKeys added=${addedKeys.length} removed=${
            removedKeys.length
          } tracked keys id=${id}`
        );
      })
  ...
  removeKeys(ids: number[]) {
    ids.forEach((id: number) => {
      ...
      this.store_
        ...
        .then((numKeys: number) => {
          boundLog(
            `removed tracked query id=${id} and ${numKeys} tracked keys`
          );
        })
        .catch((error: Error) => {
