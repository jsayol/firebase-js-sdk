...
const boundLog = log.bind(null, 'ServerCacheStore:') as (
  ...args: any[]
) => void;
const boundWarn = warn.bind(null, 'ServerCacheStore:') as (
  ...args: any[]
) => void;
...
export class ServerCacheStore {
  ...
  overwrite(node: Node, path: Path, partial: boolean): Promise<any> {
    ...
    if (partial) {
      ...
      node.forEachChild(KEY_INDEX, (childKey: string, childNode: Node) => {
        ...
        numLeafs += ServerCacheStore.saveNode_(
          childNode,
          childPath,
          writeBatch
        );
    ...
    return writeBatch
      ...
      .then(() => {
        boundLog(
          `overwrite saved ${numLeafs} leaf nodes path=${path} partial=${partial}`
        );
      })
      .catch((error: Error) => {
  ...
  getAtPath(path: Path, logging = false): Promise<Node> {
    ...
    return this.store_
        ...
        if (items.length === 0) {
          ...
          if (items.length === 1 && baseKey === items[0].key) {
            ...
            items.forEach((item: KeyValueItem<any>) => {
              let child = data;
              const subpath = item.key
                .substring(baseKey.length)
                .split('/')
      ...
      .then((node: Node) => {
        if (logging) {
          boundLog(
            `getAtPath loaded ${node.numChildren()} children path=${path}`
          );
        }
        return node;
      })
  ...
  getForKeys(keys: string[], path: Path): Promise<Node> {
    ...
    keys.forEach((key: string) => {
      const cachePromise = this.getAtPath(path.child(key), false).then(
        (childNode: Node) => {
    ...
    return Promise.all(waitFor)
      .then(() => {
        boundLog(
          `getForKeys loaded ${node.numChildren()} children path=${path}`
        );
        return node;
      })
      .catch((error: Error) => {
  ...
  private static removeLeafNodes_(
    path: Path,
  ...
  private static setNestedData_(
    ...
    if (typeof data === 'object' && !Array.isArray(data)) {
      ...
      forEach(data, (childKey: string, childVal: any) => {
        ...
        numLeafs += ServerCacheStore.setNestedData_(
          childVal,
          childPathStr,
          writeBatch
        );
